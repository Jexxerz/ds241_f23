...
The Intercept coefficient represents the predicted number of bikes rented in winter, where the following season coefficients represent how many more expected bike rentals there will be for that season
...
```{r}
lmmultifit <- lm(cnt ~ temp + atemp + mnth + hum + windspeed, bikes)
...
For each increase in degree Celsius in temperature and "feels like" temperature, there's an expected increase of rented bikes by their corresponding coefficient. As the months go on we expect in increase in rented bikes. But as the humidity and wind speed increase, there's expected decrease in the amount of rented bikes.
...
Decreased: hum, windspeed
    
##### **(d)** Which predictors are statistically significant at the 0.05 level?
   
Temp and atemp are both statistically significant at the 0.05 level
...
pairs(~ temp + atemp + mnth + hum + windspeed, bikes, lower.panel = panel.cor)
```
...
It is not surprising that temp and atemp are so strongly correlated, but it makes sense that nothing else is linearly correlated to months because they are cyclical.

##### **(c)** Refit your regression model, but this time **omit** the `temp` variable.  Display the coefficients table for this model.

```{r}

notempfit <- lm(cnt ~ atemp + mnth + hum + windspeed, bikes)
...
pairs(~ atemp + mnth + hum + windspeed, bikes, lower.panel = panel.cor)
```
...
Without temp, atemp becomes noticeably larger and also loses its statistical significance. Because atemp no longer has a partner variable of which it can be explained with, it loses its accuracy.
